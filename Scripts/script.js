...
// 7. Progress Bar Animation for Ecology Section
    ...
    const progressObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (target) {
                    // Wymuszenie re-flow dla pewności, że animacja wystartuje
                    void bar.offsetWidth; 
                    setTimeout(() => {
                        bar.style.width = target + '%';
                        
                        // Pokaż procenty (jeśli są wewnątrz paska)
...
// Inicjalizacja: sprawdzamy stan dokumentu
